...
<div class="max-w-2xl mx-auto space-y-6" x-data="{
    ...
    <div class="flex items-center space-x-4">
        <a href="{{ route('websites.index') }}" class="p-2.5 rounded-xl bg-hostinger-card border border-hostinger-border text-slate-500 hover:text-slate-900 transition shadow-md">
            <i data-lucide="arrow-left" class="w-5 h-5"></i>
        </a>
        <div>
            <h2 class="text-2xl font-extrabold text-slate-900 tracking-tight">Upload New Website</h2>
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Upload a local ZIP archive or import directly from a GitHub repository</p>
        </div>
    </div>

    <!-- Validation Error Alert -->
    @if ($errors->any())
        <div class="p-4 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-700 shadow-md">
    ...
    @if (session('error'))
        <div x-data="{ show: true }" x-show="show" class="p-4 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-700 shadow-md">
            <div class="flex items-start justify-between space-x-3">
                <div class="flex items-start space-x-3">
                    <i data-lucide="x-circle" class="w-5 h-5 mt-0.5 shrink-0"></i>
                    <span class="text-sm font-medium">{{ session('error') }}</span>
                </div>
                <button @click="show = false" class="shrink-0 text-rose-700 hover:text-rose-200">
    ...
    <div class="grid grid-cols-2 p-1.5 bg-hostinger-card border border-hostinger-border rounded-2xl gap-2 shadow-lg">
        <button type="button" @click="sourceType = 'zip'"
                :class="sourceType === 'zip' ? 'bg-indigo-600 text-white font-bold shadow-md shadow-indigo-600/30' : 'text-slate-500 hover:text-slate-900'"
        ...
        <button type="button" @click="sourceType = 'github'"
                :class="sourceType === 'github' ? 'bg-indigo-600 text-white font-bold shadow-md shadow-indigo-600/30' : 'text-slate-500 hover:text-slate-900'"
                class="py-3 px-4 rounded-xl text-xs sm:text-sm transition flex items-center justify-center space-x-2">
    ...
    <div class="bg-hostinger-card border border-hostinger-border rounded-2xl p-6 sm:p-8 shadow-2xl glow-effect">
        ...
        <form method="POST" action="{{ route('websites.store') }}" enctype="multipart/form-data"
            ...
            <!-- Website Name -->
            <div>
                <label for="name" class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="name" id="name" x-model="projectName" required
                           ...
                           class="w-full pl-11 pr-4 py-3 bg-hostinger-dark border border-hostinger-border rounded-xl text-slate-900 placeholder-gray-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm transition">
                </div>
            </div>
            ...
            <div x-show="sourceType === 'zip'" x-transition>
                ...
                <div
                    ...
                    :class="isDragging ? 'border-indigo-500 bg-indigo-500/5' : 'border-hostinger-border hover:border-indigo-400 hover:shadow-xl hover:shadow-indigo-500/10'"
                ...
                >
                    ...
                    <div x-show="!fileName" class="space-y-3">
                        ...
                        <div>
                            <p class="text-slate-900 font-bold text-sm">Drop your website.zip here</p>
                            <p class="text-slate-500 text-xs mt-1">or <span class="text-indigo-600 font-semibold">click to browse</span></p>
                        </div>
                        <p class="text-[11px] text-slate-400 pt-2">Only .zip files accepted • Max 100 MB per archive</p>
                    </div>

                    <!-- File Selected State -->
                    <div x-show="fileName" class="space-y-3">
                        <div class="w-16 h-16 rounded-2xl bg-emerald-100 border border-emerald-500/20 flex items-center justify-center mx-auto">
                            <i data-lucide="file-archive" class="w-8 h-8 text-emerald-700"></i>
                        </div>
                        <div>
                            <p class="text-emerald-700 font-bold text-sm" x-text="fileName"></p>
                            <p class="text-slate-500 text-xs mt-0.5" x-text="fileSize"></p>
                        </div>
                        <p class="text-[11px] text-indigo-600">✓ ZIP selected — click to change file</p>
                    </div>
                </div>
            </div>
            ...
            <div x-show="sourceType === 'github'" x-transition class="space-y-4">
                <label for="github_url" class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1">
                ...
                <div class="relative">
                    ...
                    <input type="url" name="github_url" id="github_url" x-model="githubUrl" @input="parseGithubName()"
                           ...
                           class="w-full pl-11 pr-4 py-3 bg-hostinger-dark border border-hostinger-border rounded-xl text-slate-900 placeholder-gray-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm font-mono transition">
                </div>

                <div class="p-4 rounded-xl bg-slate-200 border border-slate-200 space-y-2 text-xs text-slate-500">
                    ...
                    <p class="text-[11px] leading-relaxed">
                        EcoHost will automatically clone and stream the <code class="text-emerald-700 font-mono">main</code> (or <code class="text-emerald-700 font-mono">master</code>) branch of your public GitHub repository, validate your static files, and deploy to EcoHost Cloud 24/7 Engine!
                    </p>
                </div>
            </div>

            <!-- Requirements Checklist -->
            <div class="p-4 rounded-xl bg-blue-500/5 border border-blue-500/20 text-xs text-slate-500 space-y-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-1.5 mt-2">
                    <div class="flex items-center space-x-2">
                        <i data-lucide="check" class="w-3.5 h-3.5 text-emerald-700 shrink-0"></i>
                    ...
                    <div class="flex items-center space-x-2">
                        <i data-lucide="x" class="w-3.5 h-3.5 text-rose-700 shrink-0"></i>
