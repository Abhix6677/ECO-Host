...
<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-5">
        ...
        <div class="shrink-0 flex items-center space-x-3">
            @if ($isTunnelConfigured)
                <div class="flex items-center space-x-2.5 px-4 py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-semibold shadow-md">
                    <span class="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse"></span>
                    <i data-lucide="server" class="w-4 h-4 text-emerald-400"></i>
                    <span>EcoHost Cloud Engine Active</span>
                </div>
            @else
                <div class="flex items-center space-x-2.5 px-4 py-2.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-semibold shadow-md">
                    <i data-lucide="alert-triangle" class="w-4 h-4 text-amber-400"></i>
                    <span>EcoHost Tunnel Offline</span>
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div class="bg-hostinger-card border border-hostinger-border rounded-2xl p-5 space-y-2 shadow-lg">
            <div class="flex items-center justify-between">
                <span class="text-xs font-semibold text-gray-400">Total Deployment Runs</span>
                <div class="p-2 rounded-xl bg-indigo-500/10 text-indigo-400">
                    <i data-lucide="history" class="w-4 h-4"></i>
                </div>
            </div>
            <p class="text-2xl font-extrabold text-white">{{ $deployments->total() }}</p>
        </div>

        <div class="bg-hostinger-card border border-hostinger-border rounded-2xl p-5 space-y-2 shadow-lg">
            <div class="flex items-center justify-between">
                <span class="text-xs font-semibold text-gray-400">Live Sites Deployed</span>
        ...
        <div class="bg-hostinger-card border border-hostinger-border rounded-2xl p-5 space-y-2 shadow-lg">
            <div class="flex items-center justify-between">
                <span class="text-xs font-semibold text-gray-400">Failed Deployments</span>
        ...
        <div class="bg-hostinger-card border border-hostinger-border rounded-2xl p-5 space-y-2 shadow-lg">
            <div class="flex items-center justify-between">
                <span class="text-xs font-semibold text-gray-400">Hosting Environment</span>
                <div class="p-2 rounded-xl bg-blue-500/10 text-blue-400">
                    <i data-lucide="cpu" class="w-4 h-4"></i>
                </div>
            </div>
            <p class="text-sm font-bold text-indigo-300 truncate" title="EcoHost Cloud Engine">
                EcoHost 24/7
            </p>
        </div>
    </div>
    ...
    <div class="p-5 rounded-2xl bg-gradient-to-r from-indigo-900/30 via-hostinger-card to-blue-900/20 border border-indigo-500/30 text-xs sm:text-sm">
        <div class="flex items-start space-x-3.5">
            <div class="p-2 rounded-xl bg-indigo-500/20 text-indigo-400 shrink-0 mt-0.5">
                <i data-lucide="info" class="w-5 h-5"></i>
            </div>
            <div class="space-y-2 text-gray-300">
                <div class="flex flex-wrap items-center justify-between gap-2">
                    <p class="font-bold text-white text-sm">Target Hosting Node: EcoHost Cloud Engine</p>
                    <span class="text-xs font-mono text-indigo-300 bg-slate-900 px-3 py-1 rounded-lg border border-slate-700">
    ...
    @if ($deployments->isEmpty())
        ...
        <div class="bg-hostinger-card border border-hostinger-border rounded-2xl p-16 text-center shadow-xl">
            <div class="w-20 h-20 rounded-2xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center mx-auto mb-5">
                <i data-lucide="rocket" class="w-10 h-10 text-indigo-400"></i>
            </div>
            <h3 class="text-xl font-bold text-white mb-2">No Deployments Recorded</h3>
            <p class="text-gray-400 text-xs sm:text-sm max-w-sm mx-auto mb-6">
                Upload a website project and trigger your first deployment to view logs and history here.
            </p>
            <a href="{{ route('websites.create') }}"
               class="inline-flex items-center space-x-2 px-6 py-3 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-indigo-600/30 transition">
        ...
        <div class="bg-hostinger-card border border-hostinger-border rounded-2xl overflow-hidden shadow-2xl">
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-hostinger-border text-[11px] font-bold text-gray-400 uppercase tracking-wider bg-hostinger-dark/70">
                        ...
                        <tbody x-data="{ showLog: false, copied: false, copyLog(text) { navigator.clipboard.writeText(text); this.copied = true; setTimeout(() => this.copied = false, 2000); } }" 
                                        ...
                                        <div>
                                            @if ($dep->website)
                                                <a href="{{ route('websites.show', $dep->website) }}" 
                                                   class="font-bold text-white hover:text-indigo-300 transition flex items-center space-x-1.5">
                                                    <span>{{ $dep->website->name }}</span>
                                                    <i data-lucide="external-link" class="w-3.5 h-3.5 text-gray-500"></i>
                                                </a>
                                            @else
                                                <span class="font-bold text-gray-400 italic">(Deleted Website)</span>
                                            @endif
                                            <p class="text-[11px] text-gray-400 font-mono mt-0.5">{{ $dep->uuid }}</p>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="py-4 px-6 text-right">
                                    @if ($dep->log_output)
                                        <button @click="showLog = !showLog"
                                                class="text-xs font-bold text-gray-300 hover:text-white bg-slate-800/90 border border-slate-700 hover:border-indigo-500 px-3.5 py-1.5 rounded-xl transition flex items-center space-x-1.5 ml-auto shadow-sm">
                            ...
                            @if ($dep->log_output)
                                <tr x-show="showLog" x-collapse>
                                    <td colspan="5" class="px-6 py-4 bg-slate-950/60">
                                        <div class="bg-black/90 border border-indigo-500/30 rounded-xl overflow-hidden shadow-xl">
                                            <div class="bg-slate-900 px-4 py-2 border-b border-slate-800 flex items-center justify-between">
                                                <div class="flex items-center space-x-2 text-xs font-mono text-gray-400">
                                                    ...
                                                    <span>ecohost_cloud_deployment_run_{{ substr($dep->uuid, 0, 8) }}.log</span>
                                                </div>
                                                <button @click="copyLog(@js($dep->log_output))"
                                                        class="text-[11px] font-mono text-gray-400 hover:text-white bg-slate-800 px-2.5 py-1 rounded-lg border border-slate-700 hover:border-indigo-500 transition flex items-center space-x-1">
                                                    <i data-lucide="copy" class="w-3 h-3 text-indigo-400"></i>
                                                    <span x-text="copied ? 'Copied' : 'Copy Log'"></span>
                                                </button>
                                            </div>
                                            <div class="p-4 font-mono text-xs text-emerald-400 whitespace-pre-wrap leading-relaxed max-h-64 overflow-y-auto bg-slate-950">
                                                {{ $dep->log_output }}
                                            </div>
                                        </div>
                                    </td>
