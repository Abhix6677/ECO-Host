...
<div class="w-full max-w-md">
    <div class="text-center mb-8">
        ...
        <p class="text-sm text-slate-600 font-medium mt-2">Sign in to manage &amp; deploy your web projects</p>
    </div>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-8 shadow-xl">
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5" x-data="{ isSubmitting: false }" @submit="isSubmitting = true">
            ...
            <div class="flex items-center justify-between">
                <label class="flex items-center space-x-2.5 cursor-pointer">
                    <input type="checkbox" name="remember" class="w-4 h-4 rounded bg-white border-slate-300 text-indigo-600 focus:ring-indigo-500 focus:ring-offset-0">
        ...
        <div class="mt-8 pt-6 border-t border-slate-200 text-center">
            <p class="text-xs text-slate-600">
                ...
                <a href="{{ route('register') }}" class="font-bold text-indigo-600 hover:text-indigo-800 transition">Create Account</a>
            </p>
        </div>
    </div>
</div>
